...
def get_args_parser():
    ...
    parser.add_argument("-model_root_dir", default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Models",
    ...
    parser.add_argument("-img_root", type=str, default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/OWDUTS_class/img")
    parser.add_argument("-co_gt_root", type=str, default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/OWDUTS_class/gt")

    parser.add_argument("-img_root_coco", type=str, default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/OWCoCoseg/img")
    parser.add_argument("-co_gt_root_coco", type=str, default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/OWCoCoseg/gt")
    parser.add_argument("-img_syn_root", type=str,
                        default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/data2/DUTS_class_syn_3/img_png_seamless_cloning_add_naive/img")
    parser.add_argument("-img_rev_syn_root", type=str,
                        default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/data2/DUTS_class_syn_3/img_png_seamless_cloning_add_naive_reverse_2/img")
    parser.add_argument("-co_gt_rev_syn_root", type=str,
                        default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Train/data2/DUTS_class_syn_3/img_png_seamless_cloning_add_naive_reverse_2/co_gt")
    parser.add_argument("-test_data_root", type=str, default="/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Dataset/Test")
    parser.add_argument("-test_datasets", nargs='+', default=["OWCoCA"])
    parser.add_argument("-save_dir", type=str, default='/mimer/NOBACKUP/groups/alvis_cvl/Fahad/vision_tasks_L/CoSaliency/Predictions')
    parser.add_argument("-train_w_coco_prob", type=float, default=0.5)
    parser.add_argument("-max_num", type=int, default=6)
    parser.add_argument("-noise_num", type=int, default=2)
...
class Criterion(nn.Module):
    ...
    def stage_loss(self, stage_co_pred, stage_bg_pred,
                   stage_com_pred, stage_com_pred_1, all_gt,
                   bg_gt, co_gt):
        ...
        self.s_com_1_bce += self.bce(stage_com_pred_1, co_gt)
    ...
    def average_loss(self, stage_num):
        ...
        self.s_com_1_bce = self.s_com_1_bce / stage_num
        # self.s_co_2_bce = self.s_co_2_bce / stage_num
        self.s_iou = self.s_iou / stage_num
        self.s_iou_com = self.s_iou_com / stage_num
        # self.s_weight_bce = self.s_weight_bce / stage_num
        # self.s_weight_bce = self.s_weight_bce / stage_num

    def __call__(self, result, co_gt, ori_co_gt):
        self.reset_loss()

        co_gt[co_gt < 0.5] = 0.
        co_gt[co_gt >= 0.5] = 1.

        ori_co_gt[ori_co_gt < 0.5] = 0.
        ori_co_gt[ori_co_gt >= 0.5] = 1.

        all_gt = co_gt + ori_co_gt
        bg_gt = 1 - all_gt
        ...
        sum_co_gt_ = ori_co_gt.view(bs, 1, -1).sum(dim=-1)
        weight_gt_ = (sum_co_gt_ > 0).to(torch.float)

        assert (weight_gt == 1 - weight_gt_).all()
        ...
        stage_com_preds_1 = result.pop('stage_com_preds_1')
        ...
        for i in range(stage_num):
            self.stage_loss(
                ...
                stage_com_preds[i], stage_com_preds_1[i],
                all_gt, bg_gt, co_gt
            )
        ...
        loss = self.f_co_bce + self.f_bg_bce + self.f_com_bce + self.f_iou + self.f_iou_com + \
               self.s_co_bce + self.s_bg_bce + self.s_com_bce + self.s_com_1_bce + \
...
def main(args):
    ...
    for epoch in range(max_epoches):
        ...
        for iteration, data_batch in enumerate(train_loader):
            ...
            ori_co_gts = Variable(data_batch["ori_co_gts"].squeeze(0).cuda())

            result = model(imgs)

            loss = cri(result, co_gts, ori_co_gts)
            ...
            logger.info('Whole iter step:{0} - epoch progress:{1}/{2} - total_loss:{3:.4f} - f_co_bce:{4:.4f} '
                        ...
                        '- s_co_bce:{9:.4f} - s_bg_bce: {10:.4f} - s_com_bce: {11:.4f} - s_com_1_bce: {12:.4f} '
                        ...
                        ' batch_size: {16}'.format(whole_iter_num, epoch, max_epoches,
                                                   ...
                                                   cri.s_com_bce, cri.s_com_1_bce,
                                                   cri.s_iou, cri.s_iou_com, cri.s_weight_bce, co_gts.shape[0]))
        ...
        data_loader = test_loaders['OWCoCA']

        save_root = os.path.join(args.save_dir, 'OWCoCA', '{}_iter{}'.format(model_name, whole_iter_num))
        print("evaluating on {}".format('OWCoCA'))
        ...
        label_data_dir = os.path.join(args.test_data_root, 'OWCoCA', 'GroundTruth')
